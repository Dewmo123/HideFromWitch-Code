...
namespace DewmoLib.Dependencies
{
    ...
    public class Injector : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            EventBus.AddListener<DependencyEvent>(HandleDependencyEvent);
            ...
            IEnumerable<INeedInject> injectables = GetMonoBehaviours()
                .Where(IsInjectable)
                .OfType<INeedInject>();
        ...
        {
            EventBus.RemoveListener<DependencyEvent>(HandleDependencyEvent);
        }

        private void HandleDependencyEvent(DependencyEvent @event)
        {
            Inject(@event.needInject);
            @event.needInject.WaitInject.SetResult();
        }

        private void Inject(INeedInject injectableMono)
        {
